'''
SHA-RNN implementation from Merrity's repo, refactored for TAPE
Felix June 2020

Hidden state resetting after SEP token is untested - more complex than
AWD-LSTM, because SHA-RNN keeps a memory of previous segments to perform attention on.
Therefore we reset the hidden state + mask the memory.
'''
...
class SHARNN(nn.Module):
    ...
    def forward(self, inputs, attn_mask, mem=None, hidden=None, input_ids =  None, input_ids_mem = None):
        ...
        if self.attn is not None:
            ...
            if input_ids is not None: #fix attention mask for SEP tokens
                #update id memory
