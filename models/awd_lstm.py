...
API based on TAPE
Felix June 2020
NOTE LSTM output sizes as given by system are not true, because the operations of the custom lstm cell are 
performed at once for all gates, with weights from the same linear layer.
Doesn't matter, just don't get confused when printing the model. Don't know how built-in LSTM version handles that
'''
import torch
